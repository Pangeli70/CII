...
<section>`);
        if (testLogger.hasErrors) {
            ...
            <span style="color: red;">`);
                        ...
                        const codedMessage = JSON.stringify(event.result.codedMessage).replaceAll(" ", " ")

                        r.push(r.push(codedMessage););

                        let codedMessage2 = event.result.codedMessage.template;
                        ...
                        if (event.result.codedMessage.params && event.result.codedMessage.params.length > 0) {
                            ...
                            for (let i = 0; i != params.length; i++) {
                                ...
                                r.push(codedMessage2 = codedMessage2.replaceAll(placeholder, param));
    ...
    <p>
        <code>`);
            r.push(JSON.stringify(testLogger, undefined, '  ').replaceAll('\n', '<br>').replaceAll(' ', ' '));
            r.push(`</code>
    </p>`);
        } else {
